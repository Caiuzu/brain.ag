...
export default class DashboardController {
  public async getTotalFarmsAndArea({ response }: HttpContextContract) {
    const totalFarms = (await Database.from('farms')
      .count('* as total')
      .first()) as AggregationResult
    const totalArea = (await Database.from('farms')
  ...
  public async getFarmsByState({ response }: HttpContextContract) {
    const queryResult = await Farm.query().groupBy('state').count('* as total').select('state')
